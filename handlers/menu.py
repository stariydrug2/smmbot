...
@router.message(F.text.in_({'🧑‍💻 Поддержка', 'ℹ️ Помощь'}))
...
async def payment_entry(message: Message, queries, subscription_service: SubscriptionService) -> None:
    ...
    if status.get('ends_at_human'):
        text += (
            f"\n\n<b>Текущий статус:</b> {escape_html(str(status.get('status')))}"
            f"\n<b>Действует до:</b> {escape_html(str(status.get('ends_at_human')))}"
        )
...
async def callback_go_menu(callback: CallbackQuery, state: FSMContext, settings: Settings) -> None:
    ...
    if callback.message:
        await callback.message.answer(
            MENU_TEXT,
            reply_markup=main_menu_keyboard(is_admin=bool(callback.from_user and callback.from_user.id in settings.admin_ids)),
        )
