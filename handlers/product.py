...
from utils.texts import HELP_TEXT, MENU_TEXT, PAYMENT_MENU_TEXT, SUBSCRIPTION_REQUIRED_TEXT
...
async def free_analysis_goal(
    ...
    if bool(user.get('used_free_analysis')):
        ...
        await callback.answer()
        return
    if await state.get_state() == GenerationStates.waiting_for_analysis_subscription.state:
        await callback.answer('Разбор уже готовится или ждёт проверки подписки', show_alert=True)
        return
    data = await state.get_data()
    posts = [str(post).strip() for post in data.get('analysis_posts') or [] if str(post).strip()]
    ...
    await callback.answer('Начинаю разбор постов...' if len(posts) > 1 else 'Начинаю разбор поста...')
    await queries.set_user_subscription(callback.from_user.id, False)
    await state.set_state(GenerationStates.waiting_for_analysis_subscription)
    await callback.message.answer(
    ...
    refreshed_user = await queries.get_user_by_telegram_id(callback.from_user.id)
    if refreshed_user and bool(refreshed_user.get('is_subscribed')):
        await deliver_pending_free_analysis(
            callback.message,
            state,
            queries,
            int(user['id']),
        ...
        await callback.message.answer(
            '<b>Разбор готов.</b>\n\nПодпишись на канал и нажми «Проверить подписку» — сразу покажу результат.',
            reply_markup=subscription_keyboard(settings.channel_link),
        )
