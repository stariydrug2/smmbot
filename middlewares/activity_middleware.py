...
class ActivityMiddleware(BaseMiddleware):
    ...
    async def _log_event(self, event: TelegramObject) -> None:
        ...
        if not extracted:
            return
        if extracted['event_name'] in {'message_sent', 'voice_sent', 'photo_sent', 'document_sent'}:
            return
    ...
    def _classify_message(message: Message, text: str) -> str:
        ...
        if 'админка' in lower:
            return 'admin_opened'
        if lower == '📣 мой канал':
            return 'channel_section_opened'
        if lower == '✍️ создать':
            return 'content_create_opened'
        if lower == '🗓 план':
            return 'content_plan_opened'
        if lower == '🔎 анализ':
            return 'analysis_section_opened'
        if lower == '📈 результаты':
            return 'results_opened'
    ...
    def _classify_callback(data: str) -> str:
        ...
        if data == 'analysis:posts':
            return 'free_analysis_opened'
        if data.startswith('analysis_goal:'):
            return 'free_analysis_started'
        if data.startswith('analysis:'):
            return 'free_analysis_step_clicked'
        if data in {'go:menu', 'nav:main'}:
            return 'menu_opened'
        if data.startswith('trial:'):
            return f"trial_action_{data.split(':', 1)[1]}"
        if data.startswith('channel:'):
            return f"channel_action_{data.split(':', 1)[1]}"
        if data.startswith('create:'):
            return f"create_action_{data.split(':', 1)[1]}"
        if data.startswith('draft:'):
            return f"draft_action_{data.split(':', 1)[1].split(':', 1)[0]}"
