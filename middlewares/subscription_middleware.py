...
class SubscriptionMiddleware(BaseMiddleware):
    """Checks channel membership and paid subscription before protected bot actions.

    Public actions remain available: /start, channel subscription check, help,
    profile, payment screens and admin commands. Everything else is blocked when
    PAYMENT_ENABLED=true and the user's trial/paid subscription has expired.
    """
    ...
    async def __call__(
        ...
        # /start and subscription-check callbacks must always be reachable.
        ...
        # ADMIN_IDS must work even before /start refreshes is_admin in DB.
        ...
        # Channel subscription gate remains first.
        if not bool(user.get('is_subscribed')):
            await self._send_channel_required(event)
            return None

        # Payment gate works only when explicitly enabled in env.
        if not self.settings.payment_enabled:
            return await handler(event, data)

        # Payment/profile/help/admin screens must stay available after expiry.
        if is_billing_exempt_event(event):
            return await handler(event, data)
