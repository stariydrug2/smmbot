from __future__ import annotations

import asyncio
import contextlib
import logging

from aiohttp import web
from aiogram import Bot, Dispatcher
from aiogram.client.default import DefaultBotProperties
from aiogram.enums import ParseMode
from aiogram.fsm.storage.memory import MemoryStorage

from config import settings
from database.db import Database
from database.product_repository import ProductRepository
from database.queries import QueryService
from handlers import register_routers
from middlewares.activity_middleware import ActivityMiddleware
from middlewares.throttle_middleware import ThrottleMiddleware
from services.content_service import ContentService
from services.competitor_service import CompetitorMonitoringService
from services.channel_service import ChannelService
from services.image_service import ImageService
from services.memory_service import MemoryService
from services.openai_service import OpenAIService
from services.payment_http_server import create_payment_app
from services.payment_service import PaymentService
from services.publishing_service import PublishingService
from services.robokassa_service import RobokassaService
from services.subscription_service import SubscriptionService
from services.trial_service import TrialService
from services.transcription_service import TranscriptionService
from services.usage_service import UsageService


async def start_http_server(app: web.Application, host: str, port: int) -> web.AppRunner:
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, host=host, port=port)
    await site.start()
    return runner


async def main() -> None:
    settings.validate()
    logging.basicConfig(
        level=getattr(logging, settings.log_level.upper(), logging.INFO),
        format='%(asctime)s | %(levelname)s | %(name)s | %(message)s',
    )

    db = Database(settings.database_path)
    await db.connect()
    await db.init_db()

    bot = Bot(token=settings.bot_token, default=DefaultBotProperties(parse_mode=ParseMode.HTML))
    dp = Dispatcher(storage=MemoryStorage())

    queries = QueryService(db)
    product_repository = ProductRepository(db)
    openai_service = OpenAIService(
        api_key=settings.openai_api_key,
        model=settings.openai_text_model,
        gemini_api_key=settings.gemini_api_key,
        gemini_model=settings.gemini_text_model,
    )
    image_service = ImageService(api_key=settings.openai_api_key, model=settings.openai_image_model)
    transcription_service = TranscriptionService(api_key=settings.openai_api_key, model=settings.openai_transcribe_model)
    memory_service = MemoryService(
        queries=queries,
        openai_service=openai_service,
        product_repository=product_repository,
    )
    subscription_service = SubscriptionService(queries=queries, settings=settings)
    await subscription_service.bootstrap()

    robokassa_service = RobokassaService(settings=settings)
    payment_service = PaymentService(
        queries=queries,
        robokassa_service=robokassa_service,
        subscription_service=subscription_service,
    )
    content_service = ContentService(queries=queries, openai_service=openai_service, memory_service=memory_service)
    usage_service = UsageService(queries=queries, repository=product_repository)
    competitor_service = CompetitorMonitoringService(
        repository=product_repository,
        queries=queries,
        content_service=content_service,
        usage_service=usage_service,
        settings=settings,
    )
    channel_service = ChannelService(repository=product_repository, settings=settings)
    trial_service = TrialService(repository=product_repository, settings=settings)
    publishing_service = PublishingService(
        repository=product_repository,
        channel_service=channel_service,
        settings=settings,
    )
    recovered = await product_repository.recover_stale_usage()
    if recovered:
        logging.getLogger(__name__).warning('Recovered %s stale usage reservations', recovered)
    recovered_schedules = await product_repository.recover_stale_schedules()
    if recovered_schedules:
        logging.getLogger(__name__).warning('Recovered %s stale schedules', recovered_schedules)
    recovered_drafts = await product_repository.recover_stale_draft_publications()
    if recovered_drafts:
        logging.getLogger(__name__).warning(
            'Marked %s interrupted immediate publications for review', recovered_drafts
        )

    dp.workflow_data.update(
        db=db,
        queries=queries,
        product_repository=product_repository,
        settings=settings,
        openai_service=openai_service,
        image_service=image_service,
        transcription_service=transcription_service,
        memory_service=memory_service,
        subscription_service=subscription_service,
        payment_service=payment_service,
        robokassa_service=robokassa_service,
        content_service=content_service,
        usage_service=usage_service,
        competitor_service=competitor_service,
        channel_service=channel_service,
        trial_service=trial_service,
        publishing_service=publishing_service,
    )

    # ActivityMiddleware is intentionally first: it records even blocked clicks,
    # so admin analytics can show where people get stuck.
    dp.message.middleware(ActivityMiddleware(queries=queries))
    dp.callback_query.middleware(ActivityMiddleware(queries=queries))

    dp.message.middleware(ThrottleMiddleware())
    dp.callback_query.middleware(ThrottleMiddleware())
    register_routers(dp)

    payment_runner: web.AppRunner | None = None
    reminder_task: asyncio.Task[None] | None = None
    scheduler_task: asyncio.Task[None] | None = None
    competitor_task: asyncio.Task[None] | None = None

    try:
        if settings.payment_enabled:
            payment_app = create_payment_app(payment_service, bot=bot)
            payment_runner = await start_http_server(payment_app, settings.webapp_host, settings.webapp_port)
            reminder_task = asyncio.create_task(subscription_service.run_reminder_loop(bot))
            logging.getLogger(__name__).info(
                'Payment HTTP server started on %s:%s', settings.webapp_host, settings.webapp_port
            )

        if settings.feature_publishing:
            scheduler_task = asyncio.create_task(publishing_service.run_scheduler(bot))

        if settings.feature_competitor_monitoring or settings.feature_proactive:
            competitor_task = asyncio.create_task(competitor_service.run_loop(bot))

        await bot.delete_webhook(drop_pending_updates=True)
        await dp.start_polling(bot)
    finally:
        if scheduler_task:
            scheduler_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await scheduler_task
        if reminder_task:
            reminder_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await reminder_task
        if competitor_task:
            competitor_task.cancel()
            with contextlib.suppress(asyncio.CancelledError):
                await competitor_task
        if payment_runner:
            await payment_runner.cleanup()
        await competitor_service.close()
        await bot.session.close()
        await db.close()


if __name__ == '__main__':
    asyncio.run(main())
