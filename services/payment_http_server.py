...
def create_payment_app(payment_service: PaymentService, bot: object | None = None) -> web.Application:
    app = web.Application()
...
async def robokassa_result_handler(request: web.Request) -> web.Response:
    ...
    ok, message = await payment_service.process_result_callback(payload, bot=request.app.get('bot'))
...
<html lang="ru">
  ...
  <body style="font-family:Arial,sans-serif;padding:32px;max-width:720px;margin:0 auto;">
    ...
    <p>Вернитесь в Telegram-бота. Лимиты начислятся автоматически после обработки ResultURL. Если статус ещё не обновился, нажмите кнопку проверки статуса оплаты.</p>
  </body>
</html>'''
