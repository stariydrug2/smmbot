...
class OpenAIService:
    """
    Текстовый сервис с Gemini как основным провайдером и Polza.ai как fallback.

    Vision-запросы остаются на Polza.ai, как и отдельные сервисы изображений и
    транскрибации.
    ...
    def __init__(
        ...
        self.model = model
        self.gemini_client = (
            AsyncOpenAI(api_key=gemini_api_key, base_url=GEMINI_BASE_URL, max_retries=0)
            if gemini_api_key
            else None
        )
        self.gemini_model = gemini_model
        self._last_provider: ContextVar[str] = ContextVar('last_text_provider', default='unknown')
    ...
    async def generate_text(
        ...
        if self.gemini_client:
            try:
                result = await self._generate_text_with_client(
                    client=self.gemini_client,
                    model=self.gemini_model,
                    prompt=prompt,
                    system_prompt=system_prompt,
                )
                self._last_provider.set(f'gemini:{self.gemini_model}')
                logger.info('Text generation completed: provider=gemini model=%s', self.gemini_model)
                return result
            except Exception as exc:
                logger.warning(
                    "Gemini text generation failed, falling back to Polza: model=%s error=%s",
                    self.gemini_model,
                    repr(exc),
                )
        ...
        try:
            ...
            self._last_provider.set(f'polza:{self.model}')
            logger.info('Text generation completed: provider=polza model=%s', self.model)
            return result
        except Exception as exc:
            logger.exception(
    ...
    async def _generate_text_with_client(
        self,
        client: AsyncOpenAI,
        model: str,
        prompt: str,
