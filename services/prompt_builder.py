...
def _format_profile(profile: dict[str, Any] | None) -> str:
    ...
    parts = [
        ...
        f"Продукты/услуги: {profile.get('products') or '-'}",
        f"Ключевые темы: {profile.get('key_topics') or '-'}",
        f"Рубрики: {profile.get('content_rubrics') or '-'}",
        f"Нежелательные темы: {profile.get('undesired_topics') or '-'}",
        f"Типичный CTA: {profile.get('typical_cta') or '-'}",
        f"Дополнительные инструкции: {profile.get('additional_instructions') or '-'}",
        f"Визуальный стиль: {profile.get('visual_instructions') or '-'}",
    ]
    return '\n'.join(parts)
...
def build_ideas_prompt(profile: dict[str, Any] | None, memory_summary: str, examples: list[str], user_request: str, **kwargs: Any) -> str:
    return (
        f"{_compose_context(profile, memory_summary, examples)}\n\n"
        f"ЗАДАЧА: придумай идеи Telegram-постов.\nКонтекст: {user_request}\n"
        f"Параметры:\n{_options_block(**kwargs)}\n\n"
        "Дай 5 конкретных идей. Для каждой: тема, короткий угол подачи, зачем это читать и возможный CTA."
    )
