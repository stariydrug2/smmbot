...
class PaymentService:
    ...
    async def create_payment_for_plan(self, user_id: int, plan_code: str) -> dict[str, Any]:
        if not self.robokassa_service.settings.payment_enabled:
            raise RuntimeError('Онлайн-оплата временно выключена. Напишите в поддержку.')
        ...
        try:
            ...
            await self.queries.update_payment(payment_id, status='failed')
            await self.queries.log_payment_event(payment_id, 'invoice_failed', {'error': str(exc)})
            raise
    ...
    async def process_result_callback(self, payload: dict[str, Any], bot: Any | None = None) -> tuple[bool, str]:
        ...
        if payment.get('status') != 'paid' or not await self._payment_was_applied(payment):
            applied = await self.subscription_service.activate_plan_from_payment(
            ...
            await self.queries.log_payment_event(
                int(payment['id']), 'result_url_paid' if applied else 'result_url_duplicate', payload
            )
            if applied:
    ...
    async def _payment_was_applied(self, payment: dict[str, Any]) -> bool:
        subscription = await self.queries.get_user_subscription(int(payment['user_id']))
        return bool(subscription and int(subscription.get('last_payment_id') or 0) == int(payment['id']))
        ...
        try:
            await bot.send_message(
                ...
                text=(
                    ...
                    'лимиты начислены. Откройте “Результаты”, чтобы посмотреть остаток доступа.'
                ),
            )
        except Exception:
