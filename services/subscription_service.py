...
class SubscriptionService:
    ...
    async def get_status(self, user_id: int) -> dict[str, object]:
        ...
        return {
            ...
            'plan_title': subscription.get('plan_title') or '—',
    ...
    async def can_use_bot(self, user_id: int) -> bool:
        ...
        return str(status.get('status')) in {'trial', 'active', 'lifetime', 'admin'}

    async def activate_plan_from_payment(
        self,
        user_id: int,
        plan_id: int,
        payment_id: int,
        ...
        limits = self._parse_plan_limits(plan)
        now = datetime.now(timezone.utc)

        async with self.queries.db.transaction() as connection:
            payment = await (await connection.execute(
                'SELECT * FROM payments WHERE id = ? AND user_id = ? AND plan_id = ?',
                (payment_id, user_id, plan_id),
            )).fetchone()
            if not payment:
                raise RuntimeError('Платёж не найден или не принадлежит тарифу.')
            subscription = await (await connection.execute(
                'SELECT * FROM user_subscriptions WHERE user_id = ?',
                (user_id,),
            )).fetchone()
            already_applied = bool(subscription and int(subscription['last_payment_id'] or 0) == payment_id)
            if already_applied:
                await connection.execute(
                    '''
                    UPDATE payments SET status = 'paid', paid_at = COALESCE(paid_at, ?),
                        provider_payload_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?
                    ''',
                    (paid_at, provider_payload_json, payment_id),
                )
                return False

            current_end = self._parse_dt(subscription['ends_at'] if subscription else None)
            current_status = str(subscription['status'] if subscription else '')
            base_dt = current_end if current_end and current_end > now and current_status == 'active' else now
            starts_at = (subscription['starts_at'] if subscription else None) or now.isoformat()
            ...
            if limits:
                columns = ', '.join(f'{field} = COALESCE({field}, 0) + ?' for field in limits)
                await connection.execute(
                    f'''
                    UPDATE user_limits
                    SET {columns},
                        reset_at = CASE
                            WHEN reset_at IS NULL OR datetime(reset_at) < datetime(?) THEN ?
                            ELSE reset_at
                        END,
                        updated_at = CURRENT_TIMESTAMP
                    WHERE user_id = ?
                    ''',
                    [*limits.values(), ends_at.isoformat(), ends_at.isoformat(), user_id],
                )
            await connection.execute(
                '''
                UPDATE user_subscriptions
                SET plan_id = ?, status = 'active', starts_at = ?, ends_at = ?, last_payment_id = ?,
                    reminder_sent_at = NULL, updated_at = CURRENT_TIMESTAMP
                WHERE user_id = ?
                ''',
                (plan_id, starts_at, ends_at.isoformat(), payment_id, user_id),
            )
            await connection.execute(
                '''
                UPDATE users
                SET current_tariff = ?, tariff_started_at = ?, tariff_expires_at = ?,
                    is_premium = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                ''',
                (str(plan.get('code') or ''), now.isoformat(), ends_at.isoformat(), int(plan.get('code') == 'premium'), user_id),
            )
            await connection.execute(
                '''
                UPDATE payments
                SET status = 'paid', paid_at = ?, provider_payload_json = ?, updated_at = CURRENT_TIMESTAMP
                WHERE id = ?
                ''',
                (paid_at, provider_payload_json, payment_id),
            )
    ...
    async def grant_tariff_by_username(self, username: str, plan_code: str) -> dict[str, object] | None:
        user = await self._get_user_by_username(username)
        if not user:
            return None
        plan = await self.queries.get_plan_by_code(plan_code)
        if not plan or not bool(plan.get('is_active')):
            raise RuntimeError('Тариф не найден или отключён.')
        ...
        await self._grant_plan_limits(user_id, plan, now.isoformat(), ends_at.isoformat())
        await self.queries.log_admin_event('INFO', 'grant_tariff', f'username={normalize_username(username)} plan={plan_code}')
    ...
    async def _grant_plan_limits(self, user_id: int, plan: dict[str, Any], starts_at: str, ends_at: str | None) -> None:
        limits = self._parse_plan_limits(plan)

        await self.queries.add_user_limits(user_id, limits, reset_at=ends_at)
        await self.queries.update_user_tariff(
            user_id=user_id,
            tariff_code=str(plan.get('code') or ''),
            started_at=starts_at,
            expires_at=ends_at,
            is_premium=str(plan.get('code') or '') == 'premium',
        )

    @staticmethod
    def _parse_plan_limits(plan: dict[str, Any]) -> dict[str, int]:
        limits_raw = plan.get('limits_json') or '{}'
        try:
            parsed = json.loads(str(limits_raw))
        ...
        return {
            ...
            if key in LIMIT_FIELDS and int(value or 0) > 0
        }
    ...
    def _has_admin_or_env_lifetime_access(self, user: dict[str, Any] | None) -> bool:
        ...
        lifetime_usernames = getattr(self.settings, 'lifetime_access_usernames', set())
        return bool(username and username in lifetime_usernames)
    ...
    def _lifetime_status(user: dict[str, Any] | None, status: str = 'lifetime') -> dict[str, object]:
        return {
            ...
            'days_left': None,
            'hours_left': None,
            'plan_title': 'Админ-доступ' if status == 'admin' else 'Доступ без ограничения по сроку',
            'plan_code': status,
            'reminder_sent_at': None,
