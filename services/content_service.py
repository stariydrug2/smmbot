...
class ContentService:
    ...
    async def analyze_post(self, user_id: int, post_text: str, post_goal: str | None) -> str:
        ...
        prompt = (
            ...
            "ЗАДАЧА: сделай бесплатный экспресс-анализ Telegram-поста.\n"
            "Не переписывай весь пост. Нужно показать ценность анализа, но оставить логичный следующий шаг - платное улучшение.\n\n"
            f"Цель поста: {post_goal or 'не указана'}\n\n"
            f"Пост пользователя:\n{post_text}\n\n"
            "Формат ответа:\n"
            "Оценка: X/10\n"
            "Главная проблема:\n"
            "Почему могут не дочитать:\n"
            "Что мешает восприятию:\n"
            "Что усилить в структуре:\n"
            "Что сделать с началом:\n"
            "CTA:\n"
            "Пример нового начала:\n"
            "Вывод:"
        )
