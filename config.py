...
class Settings:
    ...
    lifetime_access_usernames_raw: str = field(default_factory=lambda: os.getenv('LIFETIME_ACCESS_USERNAMES', ''))
    ...
    reminder_loop_interval_seconds: int = field(default_factory=lambda: int(os.getenv('SUBSCRIPTION_REMINDER_LOOP_SECONDS', '900')))
    ...
    @property
    def lifetime_access_usernames(self) -> set[str]:
        return {
            v.strip().lstrip('@').lower()
            for v in self.lifetime_access_usernames_raw.split(',')
            if v.strip()
        }
