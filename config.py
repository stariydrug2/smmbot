...
class Settings:
    bot_token: str = field(
        default_factory=lambda: (
            _first_env('KONTUR_BOT_TOKEN', 'TELEGRAM_BOT_TOKEN', 'TELEGRAM_TOKEN', 'BOT_API_TOKEN', 'BOT_TOKEN')
        )
    )
    ...
    def validate(self) -> None:
        ...
        if missing:
            visible_keys = sorted(
                ...
                if any(marker in key.upper() for marker in ('BOT', 'TOKEN', 'TELEGRAM', 'OPENAI', 'POLZA'))
            )
            hint = (
                'Visible related env keys: '
                + (', '.join(visible_keys) if visible_keys else 'none')
                + '. Expected one of KONTUR_BOT_TOKEN, TELEGRAM_BOT_TOKEN, TELEGRAM_TOKEN, BOT_API_TOKEN, BOT_TOKEN.'
            )
