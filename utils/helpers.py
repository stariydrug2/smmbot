...
def is_subscription_related_event(event: TelegramObject) -> bool:
    """Allow onboarding/subscription checks to pass before billing checks."""
...
def is_billing_exempt_event(event: TelegramObject) -> bool:
    """Events that should stay доступными even when paid subscription is expired.

    This includes payment flow, profile access, help, start/menu navigation and
...
def render_model_text(text: str | None) -> str:
    ...
    - escapes raw HTML
    - turns markdown headings into bold lines
    - turns **bold** fragments into <b>...</b>
    - normalizes bullets
    """
    raw = (text or '').strip()
    if not raw:
