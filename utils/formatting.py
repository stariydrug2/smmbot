...
def format_profile(
    ...
    for payment in (last_payments or [])[:3]:
        payment_lines.append(
            f"• {escape_html(payment.get('plan_title') or payment.get('plan_code') or 'Платёж')} — {payment.get('amount')} ₽ — {escape_html(_payment_status_label(payment.get('status')))}"
        )
...
def format_payment_history(payments: list[dict]) -> str:
    ...
    for payment in payments:
        lines.append(
            f"<b>#{payment['id']}</b> · {escape_html(payment.get('plan_title') or payment.get('plan_code') or 'Платёж')} · "
            f"{payment.get('amount')} ₽ · {escape_html(_payment_status_label(payment.get('status')))} · {escape_html(payment.get('created_at') or '—')}"
        )
    return '\n'.join(lines)
...
def format_payment_details(payment: dict) -> str:
    lines = [
        ...
        f"<b>Создан:</b> {escape_html(payment.get('created_at') or '—')}",
