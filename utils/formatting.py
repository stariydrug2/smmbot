...
def format_profile(
    ...
    if status.get('ends_at_human') == 'Без ограничения':
        remaining_line = '<b>Осталось:</b> Без ограничения'
    elif hours_left is None:
        remaining_line = '<b>Осталось часов:</b> —'
    elif int(hours_left) <= 24:
        remaining_line = f'<b>Осталось часов:</b> <b>{hours_left}</b>'
    else:
        remaining_line = f'<b>Осталось часов:</b> {hours_left}'
