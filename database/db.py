...
class Database:
    ...
    async def _run_migrations(self) -> None:
        ...
        ALTER TABLE users ADD COLUMN used_free_analysis INTEGER DEFAULT 0;
        ALTER TABLE users ADD COLUMN current_tariff TEXT;
        ALTER TABLE users ADD COLUMN tariff_started_at TEXT;
        ALTER TABLE users ADD COLUMN tariff_expires_at TEXT;
        ALTER TABLE users ADD COLUMN is_premium INTEGER DEFAULT 0;
        ALTER TABLE subscription_plans ADD COLUMN plan_type TEXT DEFAULT 'subscription_30_days';
        ALTER TABLE subscription_plans ADD COLUMN description TEXT;
        ALTER TABLE subscription_plans ADD COLUMN limits_json TEXT;
        """
        ...
        columns = {
            str(row['name'])
            for row in await self.fetchall('PRAGMA table_info(analyses)')
        }
        if 'posts_count' not in columns:
            await self.execute('ALTER TABLE analyses ADD COLUMN posts_count INTEGER DEFAULT 1')
        if 'delivered_at' not in columns:
            await self.execute('ALTER TABLE analyses ADD COLUMN delivered_at TEXT')
            # Analyses created before this funnel were already shown immediately.
            await self.execute(
                'UPDATE analyses SET delivered_at = created_at WHERE delivered_at IS NULL'
            )
        await self.execute(
