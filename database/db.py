...
class Database:
    ...
    async def init_db(self) -> None:
        ...
        CREATE TABLE IF NOT EXISTS competitor_posts (
            ...
            notified_strong_at TEXT,
            first_seen_at TEXT DEFAULT CURRENT_TIMESTAMP,
            updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (source_id) REFERENCES competitor_sources(id) ON DELETE CASCADE,
            UNIQUE (source_id, external_post_id)
        );

        CREATE TABLE IF NOT EXISTS competitor_monitor_settings (
            user_id INTEGER PRIMARY KEY,
            notify_mode TEXT NOT NULL DEFAULT 'strong',
            pulse_enabled INTEGER NOT NULL DEFAULT 1,
            weekly_plan_enabled INTEGER NOT NULL DEFAULT 0,
            last_pulse_week TEXT,
            last_plan_week TEXT,
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            updated_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE
        ...
        CREATE TABLE IF NOT EXISTS competitor_reports (
            ...
            created_at TEXT DEFAULT CURRENT_TIMESTAMP,
            FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
            UNIQUE (user_id, report_type, cache_key)
        );
        ...
        CREATE INDEX IF NOT EXISTS idx_competitor_posts_source ON competitor_posts(source_id, external_post_id);
        CREATE INDEX IF NOT EXISTS idx_competitor_posts_published ON competitor_posts(published_at);
        CREATE INDEX IF NOT EXISTS idx_competitor_reports_user ON competitor_reports(user_id, report_type);
    ...
    async def _run_migrations(self) -> None:
        ...
        await self._add_column_if_missing('competitor_sources', 'initialized_at', 'TEXT')
        await self._add_column_if_missing('competitor_sources', 'last_success_at', 'TEXT')
        await self._add_column_if_missing('competitor_sources', 'last_error', 'TEXT')
