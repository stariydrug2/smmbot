...
class QueryService:
    ...
    async def add_user_example(self, user_id: int, example_text: str, channel_id: int | None = None) -> None:
        await self.db.execute(
            'INSERT INTO user_examples (user_id, channel_id, example_text) VALUES (?, ?, ?)',
            (user_id, channel_id, example_text),
        )

    async def clear_user_examples(self, user_id: int, channel_id: int | None = None) -> None:
        if channel_id is None:
            await self.db.execute('DELETE FROM user_examples WHERE user_id = ? AND channel_id IS NULL', (user_id,))
        else:
            await self.db.execute(
                'DELETE FROM user_examples WHERE user_id = ? AND channel_id = ?',
                (user_id, channel_id),
            )

    async def get_user_examples(
        self,
        user_id: int,
        limit: int = 5,
        ...
        if channel_id is None:
            rows = await self.db.fetchall(
                'SELECT example_text FROM user_examples WHERE user_id = ? AND channel_id IS NULL ORDER BY id DESC LIMIT ?',
                (user_id, limit),
            )
        else:
            rows = await self.db.fetchall(
                '''
                SELECT example_text FROM user_examples
    ...
    async def has_limit(self, user_id: int, field: str) -> bool:
        ...
        row = await self.db.fetchone(
            f'''SELECT CASE
                    WHEN reset_at IS NOT NULL AND datetime(reset_at) <= CURRENT_TIMESTAMP THEN 0
                    ELSE COALESCE({field}, 0)
                END AS value
    ...
    async def consume_limit(self, user_id: int, field: str, amount: int = 1) -> bool:
        ...
        cursor = await self.db.execute(
            ...
            WHERE user_id = ? AND COALESCE({field}, 0) >= ?
              AND (reset_at IS NULL OR datetime(reset_at) > CURRENT_TIMESTAMP)
            ''',
    ...
    async def add_analysis(
        ...
        cursor = await self.db.execute(
            ...
            INSERT INTO analyses (
                user_id, original_text, post_goal, score, result_text, is_free, posts_count
            )
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ''',
            (user_id, original_text, post_goal, score, result_text, int(is_free), max(1, posts_count)),
        )
        return int(cursor.lastrowid)

    async def get_pending_free_analysis(self, user_id: int) -> Optional[dict[str, Any]]:
        row = await self.db.fetchone(
            '''
            SELECT *
            FROM analyses
            WHERE user_id = ? AND is_free = 1 AND delivered_at IS NULL
            ORDER BY id DESC
            LIMIT 1
            ''',
            (user_id,),
    ...
    async def mark_analysis_delivered(self, analysis_id: int, user_id: int) -> bool:
        cursor = await self.db.execute(
            '''
            UPDATE analyses
    ...
    async def get_admin_stats(self) -> dict[str, Any]:
        ...
        active = await self.db.fetchone(
            ...
            FROM (
                SELECT user_id FROM generation_history WHERE created_at >= datetime('now', '-7 days')
                UNION
                SELECT user_id FROM analyses WHERE created_at >= datetime('now', '-7 days')
                UNION
    ...
    async def update_user_subscription(self, user_id: int, **fields: Any) -> None:
        if not fields:
            return
        allowed = {'plan_id', 'status', 'starts_at', 'ends_at', 'auto_renew', 'last_payment_id', 'reminder_sent_at'}
        clean = {key: value for key, value in fields.items() if key in allowed}
        if not clean:
            return
        columns = ', '.join([f'{key} = ?' for key in clean])
    ...
    async def update_payment(self, payment_id: int, **fields: Any) -> None:
        if not fields:
            return
        allowed = {
            'invoice_id', 'provider_invoice_id', 'amount', 'status', 'payment_url',
            'invoice_url', 'invoice_external_id', 'description', 'shp_payload_json',
        ...
        if not clean:
            return
        columns = ', '.join([f'{key} = ?' for key in clean])
        params = list(clean.values()) + [payment_id]
